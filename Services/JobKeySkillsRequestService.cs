	...
	{
		public class JobKeySkillsRequestService(HeadHunterHttpService headHunterHttpService, ILogger<JobKeySkillsRequestService> logger) : IJobKeySkillsRequestService, IDisposable
		{
			...
			private readonly ILogger<JobKeySkillsRequestService> _logger = logger;
			...
			public async Task<Dictionary<string, int>> GetKeySkillsForJobAsync(string jobName, int vacanciesToProcessCount, CancellationToken token = default)
			{
				...
				{
					_logger.Log(LogLevel.Warning, ex, "Http request for the first page failed");
					return [];
				}
				...
				if (actualRemainingPagesCount > 0 && remainingVacanciesToProcessCount > 0)
				{
					IEnumerable<Task<VacanciesSearchResult>> pageTasks = Enumerable.Range(1, actualRemainingPagesCount)
						.Select(async pageNumber =>
						{
							...
							{
								_logger.Log(LogLevel.Warning, ex, "Http request for page {PageNumber} failed", pageNumber);
				...
				IEnumerable<Task<string[]>> skillTasks = vacanciesIds.Select(async vacancyId =>
				{
					...
					{
						_logger.Log(LogLevel.Warning, ex, "Http request for vacancy {VacancyId} failed", vacancyId);
						return [];
					}
					finally
					{
